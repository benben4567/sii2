...
class KompetensiController extends Controller
{
  ...
  {
    return view('page.sertifikasi.kompetensi_bidang.index');
  }

  public function getData(Request $request)
  {
    if ($request->ajax()) {
      $bidang = DB::table('sertifikasibidangs')->get();
      // return dd($bidang);
  ...
  public function store(Request $request)
  {
    ...
    try {
      ...
      $jml = SertifikasiBidang::where('nama_sertifikasi', '=', $request['nama_sertifikasi'])
        ->where('instruktur_id', Auth::user()->instruktur->id)->count();
      if ($jml < 1) {
        SertifikasiBidang::create([ //MODIFIKASI BAGIAN INI DENGAN MEMASUKKANYA KE DALAM VARIABLE $USER
